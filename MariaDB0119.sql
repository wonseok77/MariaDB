...
-- user table에 postcode 칼럼 추가
ALTER TABLE user
ADD postcode VARCHAR(10);

-- user table에서 postcode 칼럼 삭제
ALTER TABLE user
DROP COLUMN postcode;


-- user table에서 address 테이블의 크기 200으로 늘리기
ALTER TABLE user
...
-- DELETE 는 반드시 WHERE조건을 써야한다 아니면 데이터 다 날아간다
-- 실무에서 정말 조심히 써야한다
...
INSERT INTO board (NULL, '제목2','내용2',NOW(),102);
INSERT INTO board (NULL, '제목2','내용2',CURDATE(),102);
